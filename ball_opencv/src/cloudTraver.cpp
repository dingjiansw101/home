                          ...
                          const sensor_msgs::CameraInfoConstPtr& inputInfo_msg)
{
    ...
    currImage = cv_ptr->image.clone();//使用了imageCb获取了cv_ptr->image.clone()的信息后将其传给cloudTraver类里的一个成员currImage,
					//从而可以使用getImage()来获取currImage对应的信息，而不用在imageCb()里对图像信息进行操作，将获取与操作分离
					//如果不用面向对象的方法，要是实现这种效果，应该要定义全局变量，每调用一次imageCb()就对全局变量赋值一次，
					//然后操作
